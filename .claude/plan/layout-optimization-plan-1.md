...
<template>
  <nav
    class="fixed bottom-0 left-0 right-0 bg-background/80 backdrop-blur-md border-t md:hidden z-40"
  >
...
```vue
<!-- Drawer动画 -->
<USlideover v-model="isDrawerOpen" class="transition-transform duration-300 ease-in-out" />

<!-- 按钮悬停效果 -->
<UButton class="transition-colors duration-200 hover:bg-primary/10" />

<!-- 卡片悬停效果 -->
<div class="hover:bg-muted rounded-lg transition-colors cursor-pointer"></div>
```
...
```vue
<!-- 焦点管理 -->
<UButton class="focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2" />
